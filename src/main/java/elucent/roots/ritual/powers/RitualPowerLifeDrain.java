...
public class RitualPowerLifeDrain extends RitualPower {
	...
	public void onRightClickEntity(EntityPlayer player, World world, Entity entity){
		...
		if (entity instanceof EntityLivingBase){
			...
			((EntityLivingBase)entity).attackEntityFrom(DamageSource.wither, 4.0f);
			damageDealt += 4.0f;
			...
			for (int i = 0; i < targets.size(); i ++){
				if (targets.get(i).getUniqueID() != player.getUniqueID()){
					targets.get(i).attackEntityFrom(DamageSource.wither, 2);
					damageDealt += 4.0f;
					for (int j = 0; j < 40; j ++){
						world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, targets.get(i).posX*(i/40.0)+((40.0-i)/40)*player.posX, (targets.get(i).posY+targets.get(i).height/2.0)*(i/40.0)+((40.0-i)/40)*(player.posY+player.height/2.0), targets.get(i).posZ*(i/40.0)+((40.0-i)/40)*player.posZ, Math.pow(0.5f*(random.nextFloat()-0.5f),3.0), Math.pow(0.5f*(random.nextFloat()-0.5f),3.0), Math.pow(0.5f*(random.nextFloat()-0.5f),3.0), 0);
					}
				}
			}
			...
			for (double i = 0; i < 40; i ++){
				world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, entity.posX*(i/40.0)+((40.0-i)/40)*player.posX, (entity.posY+entity.height/2.0)*(i/40.0)+((40.0-i)/40)*(player.posY+player.height/2.0), entity.posZ*(i/40.0)+((40.0-i)/40)*player.posZ, Math.pow(0.5f*(random.nextFloat()-0.5f),3.0), Math.pow(0.5f*(random.nextFloat()-0.5f),3.0), Math.pow(0.5f*(random.nextFloat()-0.5f),3.0), 0);
			}
