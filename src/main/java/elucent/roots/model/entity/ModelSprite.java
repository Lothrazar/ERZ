...
public class ModelSprite extends ModelBase
{
  ...
  public void render(Entity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale){
	  ...
	  setRotation(body1, -2.0f*(((EntitySprite)entity).prevPitch1-((EntitySprite)entity).rotationPitch), -2.0f*(((EntitySprite)entity).prevYaw1-((EntitySprite)entity).rotationYaw), 0);
	  setRotation(body2, 0.5f*(float)Math.PI-2.0f*(((EntitySprite)entity).prevPitch2-((EntitySprite)entity).prevPitch1), -2.0f*(((EntitySprite)entity).prevYaw2-((EntitySprite)entity).prevYaw1), 0);
	  setRotation(body3, -2.0f*(((EntitySprite)entity).prevPitch3-((EntitySprite)entity).prevPitch2), 0, 2.0f*(((EntitySprite)entity).prevYaw3-((EntitySprite)entity).prevYaw2));
	  GlStateManager.rotate(-0.5f*(float)Math.PI-(float)Math.toDegrees(entity.rotationYaw), 0, 1, 0);
	  GlStateManager.rotate((float)Math.toDegrees(-entity.rotationPitch/2.0f), 1, 0, 0);
	  if (((EntitySprite)entity).twirlTimer > 0){
		  GlStateManager.rotate((float)18.0f*(20.0f-(float)((EntitySprite)entity).twirlTimer),0,0,1);
