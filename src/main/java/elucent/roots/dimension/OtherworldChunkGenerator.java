...
public class OtherworldChunkGenerator implements IChunkGenerator {
	...
	public OtherworldChunkGenerator(World world){
		this.world = world;
		passiveCreatures.add(new SpawnListEntry(EntitySheep.class,50,5,15));
		passiveCreatures.add(new SpawnListEntry(EntityWolf.class,25,5,15));
		passiveCreatures.add(new SpawnListEntry(EntityDeer.class,50,5,15));
		passiveCreatures.add(new SpawnListEntry(EntityDireWolf.class,10,5,15));
	...
	public void populate(int x, int z) {
		...
		if (b.equals(OtherworldBiomes.biomeGlowingSands)){
			if (random.nextBoolean()){
				if (getIslandCoefficient(x*16+xx,z*16+zz) > 0.025 && getBiome(x*16+xx,z*16+zz) == OtherworldBiomes.biomeGlowingSands){
					StructureBoulder.generateRock(world, x*16+xx, getHeight(getBiomeCoeff(x*16+xx,z*16+zz),getIslandCoefficient(x*16+xx,z*16+zz),x*16+xx,z*16+zz), z*16+zz, 7);
				}
			...
			if (random.nextInt(14) == 0){
				if (getIslandCoefficient(x*16+xx,z*16+zz) > 0.025 && getBiome(x*16+xx,z*16+zz) == OtherworldBiomes.biomeGlowingSands){
					StructureBoulder.generateRock(world, x*16+xx, getHeight(getBiomeCoeff(x*16+xx,z*16+zz),getIslandCoefficient(x*16+xx,z*16+zz),x*16+xx,z*16+zz), z*16+zz, 20);
				}
				xx = random.nextInt(16);
				zz = random.nextInt(16);
			}
		...
		if (getBiome(x*16+xx,z*16+zz) == OtherworldBiomes.biomeShrubland){
			if (random.nextBoolean()){
				if (getIslandCoefficient(x*16+xx,z*16+zz) > 0.025 && getBiome(x*16+xx,z*16+zz) == OtherworldBiomes.biomeShrubland){
					StructureMossyBoulder.generateRock(world, x*16+xx, getHeight(getBiomeCoeff(x*16+xx,z*16+zz),getIslandCoefficient(x*16+xx,z*16+zz),x*16+xx,z*16+zz), z*16+zz, 7);
				}
				xx = random.nextInt(16);
				zz = random.nextInt(16);
			}
