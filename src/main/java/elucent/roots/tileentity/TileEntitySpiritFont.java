	...
	public void update() {
		...
		if (getWorld().isRemote){
			if (getWorld().provider.getDimension() == Minecraft.getMinecraft().thePlayer.getEntityWorld().provider.getDimension()){
				if (ticker % 2*((150-5*(int)maxPower)/25) == 0){
		...
		if (ticker % (150-5*(int)maxPower) == 0){
			this.power = 0;
			for (int i = -5; i < 6; i ++){
				for (int j = -5; j < 6; j ++){
					for (int k = -1; k < 2; k ++){
						...
						power += Util.getNatureAmount(getWorld().getBlockState(pos));
					}
				}
			}
			...
			for (int i = 0; i < entities.size(); i ++){
				if (entities.get(i) instanceof ISprite){
					if (!((ISprite)entities.get(i)).getTargetPosition().equals(getPos())){
						...
						((ISprite)entities.get(i)).setHappiness(this.power);
						entities.get(i).heal(power/entities.get(i).getMaxHealth());
						...
						velocity = velocity.scale(1.0/20.0);
						if (getWorld().isRemote){
							if (getWorld().provider.getDimension() == Minecraft.getMinecraft().thePlayer.getEntityWorld().provider.getDimension()){
								Roots.proxy.spawnParticleMagicSparkleScalableFX(getWorld(), 40, getPos().getX()+0.5+0.1*(random.nextDouble()-0.5), getPos().getY()+0.5+0.1*(random.nextDouble()-0.5), getPos().getZ()+0.5+0.1*(random.nextDouble()-0.5), velocity.xCoord, velocity.yCoord, velocity.zCoord, 4.0f, 107, 255, 28);
							}
						}
					}
				}
