		...
		if (ticker % 5 == 0){
			if (getWorld().isBlockIndirectlyGettingPowered(getPos().down()) > 0){
				for (double i = 0; i < 720; i += 45.0){
					...
					Roots.proxy.spawnParticleMagicAuraFX(this.getWorld(), this.getPos().getX()+0.5+xShift, this.getPos().getY()+0.5, this.getPos().getZ()+0.5+zShift, 0, 0, 0, 32, 255, 255);
				}
				if (ticker % 20 == 0){
					...
					if (nearbyCreatures.size() > 0){
						for (int i = 0; i < nearbyCreatures.size(); i ++){
							nearbyCreatures.get(i).addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("speed"),25,1));
						}
					}
				}
			}
