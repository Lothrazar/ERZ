...
public class EntitySpriteProjectile  extends EntityFlying {// implements IRangedAttackMob {
    ...
    public void collideWithEntity(Entity entity){
    	if (target != null){
	    	if (entity.getUniqueID().compareTo(target.getUniqueID()) == 0){
				...
				for (int i = 0; i < 80; i ++){
					Roots.proxy.spawnParticleMagicAuraFX(getEntityWorld(), posX, posY+height/2.0f, posZ, Math.pow(1.15f*(random.nextFloat()-0.5f),3.0), Math.pow(1.15f*(random.nextFloat()-0.5f),3.0), Math.pow(1.15f*(random.nextFloat()-0.5f),3.0), 107, 255, 28);
				}
	    	}
    	}
    }
    ...
    public void onUpdate(){
    	...
    	if (target != null){
			...
			for (double i = 0; i < 3; i ++){
				...
				Roots.proxy.spawnParticleMagicAuraFX(getEntityWorld(), x, y, z, -0.125*moveVec.xCoord, -0.125*moveVec.yCoord, -0.125*moveVec.zCoord, 107, 255, 28);
			}
		}
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount){
    	if (source.getEntity() != null){
    		this.getEntityWorld().removeEntity(this);
			for (int i = 0; i < 40; i ++){
				Roots.proxy.spawnParticleMagicAuraFX(getEntityWorld(), posX, posY+height/2.0f, posZ, Math.pow(0.95f*(random.nextFloat()-0.5f),3.0), Math.pow(0.95f*(random.nextFloat()-0.5f),3.0), Math.pow(0.95f*(random.nextFloat()-0.5f),3.0), 107, 255, 28);
			}
    	}
    	return false;
    }
