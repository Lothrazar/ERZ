...
public class NoiseGenUtil {
	...
	public static float getNoise(long seed, int x, int y){
		random.setSeed(simple_hash(new int[]{(int)seed,(int)(seed << 32),(int)Math.signum(y)*512+512,(int)Math.signum(x)*512+512,x,y},6));
		return random.nextFloat();
	}
	
	public static long getSeed(int seed, int x, int y){
		return simple_hash(new int[]{seed,(int)Math.signum(y)*512+512,(int)Math.signum(x)*512+512,x,y},6);
	}
