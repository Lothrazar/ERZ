        ...
        public IMessage onMessage(final MessageSanctuaryRingFX message, final MessageContext ctx) {
			...
			for (float k = 0; k < 360; k += random.nextInt(72)){
				if (random.nextBoolean()){
					if (random.nextBoolean()){
						ParticleUtil.spawnParticlePetal(world, (float)message.posX+3.0f*(float)Math.sin(Math.toRadians(k)), (float)message.posY, (float)message.posZ+3.0f*(float)Math.cos(Math.toRadians(k)), 0, 0, 0, SpellRegistry.spell_red_tulip.red1, SpellRegistry.spell_red_tulip.green1, SpellRegistry.spell_red_tulip.blue1, 0.5f, 1.25f+5.0f*random.nextFloat(), 40);
					}
					else {
						ParticleUtil.spawnParticlePetal(world, (float)message.posX+3.0f*(float)Math.sin(Math.toRadians(k)), (float)message.posY, (float)message.posZ+3.0f*(float)Math.cos(Math.toRadians(k)), 0, 0, 0, SpellRegistry.spell_red_tulip.red2, SpellRegistry.spell_red_tulip.green2, SpellRegistry.spell_red_tulip.blue2, 0.5f, 1.25f+5.0f*random.nextFloat(), 40);
					}
				}
			}
    		return null;
