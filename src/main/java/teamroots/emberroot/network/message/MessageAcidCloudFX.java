    ...
    public static class MessageHolder implements IMessageHandler<MessageAcidCloudFX,IMessage>
    {
        ...
        public IMessage onMessage(final MessageAcidCloudFX message, final MessageContext ctx) {
			...
			for (float i = 0; i < 360; i += random.nextInt(40)){
				...
				if (random.nextBoolean()){
					ParticleUtil.spawnParticleSmoke(world, x, y, z, vx, 0.125f*(random.nextFloat()-0.5f), vz, SpellRegistry.spell_allium.red1, SpellRegistry.spell_allium.green1, SpellRegistry.spell_allium.blue1, 0.125f, 10f+random.nextFloat()*6f, 120, false);
				}
				else {
					ParticleUtil.spawnParticleSmoke(world, x, y, z, vx, 0.125f*(random.nextFloat()-0.5f), vz, SpellRegistry.spell_allium.red2, SpellRegistry.spell_allium.green2, SpellRegistry.spell_allium.blue2, 0.125f, 10f+random.nextFloat()*6f, 120, false);
				}
			}
    		return null;
        }
